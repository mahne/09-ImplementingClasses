...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance_traveled += self.get_distance_from(Point(x, y))
        self.x = x
        self.y = y
        self.moves += 1
    ...
    def get_distance_from_start(self):
        return ((self.xstart - self.x) ** 2 + (self.ystart - self.y) ** 2) ** 0.5
    ...
    def halfway_to(self,p2):
        x = (p2.x-self.x)/2+self.x
        y = (p2.y - self.y) / 2 + self.y
        return Point(x,y)
